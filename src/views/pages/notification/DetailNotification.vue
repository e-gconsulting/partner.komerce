...
export default {
  ...
  methods: {
    ...
    async getDetailNotification() {
      await this.$http_komship.get(`/v1/admin/notification-bar/detail/${this.$route.params.notification_id}`)
        .then(result => {
          ...
          const kolor = this.colorOptions.find(items => items.value === this.color)
          this.setColors(kolor)
        }).catch(err => {
          console.error(err)
        })
    },
